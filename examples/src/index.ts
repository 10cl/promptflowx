...
let runExample;
try {
  // Correctly join the path and convert to file URL
  const examplePath = path.join(
    path.dirname(url.fileURLToPath(import.meta.url)),
    "..", // Adjusting to point to the correct directory level
    exampleRelativePath
  );
  const fileURL = url.pathToFileURL(examplePath).href;

  ({ run: runExample } = await import(fileURL));
} catch (e) {
  console.log(e);
