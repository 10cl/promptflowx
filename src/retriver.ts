...
export interface VectorStoreRetrieverFields
...
export class VectorStoreRetriever extends BaseRetriever {
   ...
   * @param fields - The fields required for initializing the VectorStoreRetriever
   */
  constructor(fields: VectorStoreRetrieverFields) {
    super(fields);
  ...
  async addDataSet(documents: Document[], embeddingNumbers: number[][]): Promise<void> {
    const now = Math.floor(Date.now() / 1000);
    const savedDocs = this.prepareDocuments(documents, now);
    this.memoryStream.push(...savedDocs);
    await this.vectorStore.addVectors(embeddingNumbers, savedDocs);
  }
  ...
  > {
    ...
    for (const doc of this.memoryStream.slice(-this.k)) {
      ...
      if (bufferIdx === undefined) {
        throw new Error(
          `Found a document in the vector store that is missing required metadata. This retriever only supports vector stores with documents that have been added through the "addDocuments" method on a VectorStoreRetriever, not directly added or loaded into the backing vector store.`
        );
      }
